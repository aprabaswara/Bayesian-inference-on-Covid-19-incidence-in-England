...
sam <- jags.samples(mod,c("m","n","tau"),n.iter=10000)
str(sam)
...
lockdown_day <- julian(as.Date("2020-3-24"),origin=as.Date("2019-12-31"))[1]
horiz_axis <- c(1,lockdown_day,length(y),length(y_new))
horiz_axis <- sort(unique(horiz_axis))
vertic_axis <- c(0,max(sam$n[1:length(y)]),max(sam$m[1:length(y_new)]))
vertic_axis <- sort(unique(vertic_axis))
vertic_axis <- round(vertic_axis,0)
...
axis(1,at=horiz_axis)
axis(2,at=vertic_axis)
legend(x='topright',legend=c("New Infections", "Expected Death", "95% Credible Interval"),col=c("green", "blue","red"), lty=c(1,1,2), cex=0.55,bty='n',lwd=1)
text(x=lockdown_day+4,y=1400,label='UK Lockdown',pos=3,cex=0.7,srt=90)
